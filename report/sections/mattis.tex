...
To measure Performance Density (PD), we used the performance spread ($G_p$) from annual top 3 performances to the bottom 3 performances (e.g. Rank 33-35) of our dataset for the two time periods. Averaging for 3 athletes prevent single-athlete outlier problems. Averaging for top 3 reflects the international standard of success (Podium), while bottom 3 provide a stable and comparable baseline. we used the Spread to measure the width of the elite level performances and we use all possible informations (e.g. Ranks) of our dataset and have a measurement of polarization without assuming a normal distribution.
...
A positive $\Delta G_{\%}$ suggests a decrease in PD, while a negative $\Delta G_{\%}$ suggests a increase in PD.

To assess the statistical significance of the change in PD, we use a bootstrap resampling method with $B=10,000$. Bootstrap is non-parametric and can handle the low sample size (N=12) well suiting our data distribution.
...
If the CI includes zero, the null hypothesis of no significance change in PD cannot be rejected.
