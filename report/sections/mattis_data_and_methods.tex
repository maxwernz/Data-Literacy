...
To assess general changes in performance, the percentage difference between the medians is used, as the median represents the typical performance level and is more robust against outliers compared to the mean. Medians are determined for each group from the data pools of the respective five-year periods. The development is then calculated as a percentage difference, normalized to the baseline period. A positive value indicates an improvement in performance, while a negative value represents a decline. Statistical significance is tested using the Mann-Whitney U test, which allows for a comparison of the periods without assuming a normal distribution.
